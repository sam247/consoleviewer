import { AiLedQueriesCard } from "@/components/ai-led-queries-card";
import { TrackedKeywordsMiniCard } from "@/components/tracked-keywords-mini-card";
import type { DataTableRow } from "@/components/data-table";

export function ProjectSimplifiedTrackersSection({
  propertyId,
  queriesRows,
  siteUrl,
}: {
  propertyId: string;
  queriesRows: DataTableRow[];
  siteUrl?: string;
}) {
  return (
    <section aria-label="Trackers" className="grid grid-cols-1 lg:grid-cols-2 gap-6 min-w-0">
      <AiLedQueriesCard queries={queriesRows} siteUrl={siteUrl} className="min-h-[480px]" />
      <TrackedKeywordsMiniCard propertyId={propertyId} className="min-h-[480px]" />
    </section>
  );
}
