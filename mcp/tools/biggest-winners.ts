...
export const getBiggestWinnersTool: ToolDefinition<"get_biggest_winners"> = {
  name: "get_biggest_winners",
  description: "Biggest winning queries by click change vs previous period.",
  inputSchema: {
    type: "object",
    properties: {
  ...
  handler: async (input, context): Promise<BiggestChangesResult> => {
    ...
    const res = await readQuery<Row>(
      ...
      ORDER BY j.click_change DESC
      LIMIT 25`,
      [propertyId, w.currentStart, w.currentEnd, w.priorStart, w.priorEnd]
    );

    const data = res.rows.map(toRow);
    const summary = data.length ? `${data.length} biggest winning queries` : "No winning queries found";
    return { summary, data };
  },
};
